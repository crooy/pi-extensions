...
// Constants
// ---------------------------------------------------------------------------
export const INSTINCTS_HEADER = "## 🧠 Instincts";
 ...
 */
export function buildFactsInjectionBlock(facts, maxChars) {
    if (facts.length === 0)
        return null;
    const headerLen = `\n\n${FACTS_HEADER}\n`.length;
    const allBullets = [];
    ...
    for (const f of facts) {
        const bullet = `${f.confidence < 1 ? "." + String(f.confidence).replace(/^0\./, "") : "1."} ${f.content}`;
        const bulletLen = bullet.length + 1; // +1 for newline
        if (maxChars && charCount + bulletLen > maxChars) {
...
export function handleBeforeAgentStartInjection(event, _ctx, config, projectId, baseDir) {
    ...
    const instinctsBlock = buildInjectionBlock(instincts, config.max_injection_chars);
    ...
    const factsBlock = buildFactsInjectionBlock(allFacts, remainingChars > 0 ? remainingChars : undefined);
