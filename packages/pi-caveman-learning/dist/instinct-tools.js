...
function formatInstinct(i) {
    return `[${i.confidence.toFixed(2)}] ${i.id} (${i.domain}, ${i.scope})\n  Trigger: ${i.trigger}\n  Action: ${i.action}`;
}
const ListParams = Type.Object({
    scope: Type.Optional(StringEnum(["project", "global", "all"])),
...
const DeleteParams = Type.Object({
    ...
    scope: Type.Optional(StringEnum(["project", "global"], {
...
const MergeParams = Type.Object({
    ...
    delete_ids: Type.Array(Type.String(), {
        description: "IDs of source instincts to remove after merge (uses priority lookup)",
    }),
    delete_scoped_ids: Type.Optional(Type.Array(Type.Object({
...
export function createInstinctWriteTool(projectId, projectName, baseDir) {
    return {
        name: "instinct_write",
        ...
        async execute(_toolCallId, params, _signal, _onUpdate, _ctx) {
            ...
            const similar = findSimilarInstinct({ trigger: params.trigger, action: params.action }, allInstincts, params.id);
            if (similar) {
            ...
            if (projectId)
                generateProjectSummary(projectId, baseDir);
...
export function createInstinctReadTool(projectId, baseDir) {
    return {
        name: "instinct_read",
        label: "Read Instinct",
        ...
        async execute(_toolCallId, params, _signal, _onUpdate, _ctx) {
            ...
            return {
                content: [{ type: "text", text: formatInstinct(instinct) }],
                details: instinct,
            };
        },
    };
}
export function createInstinctListTool(projectId, baseDir) {
    return {
        name: "instinct_list",
...
export function createInstinctDeleteTool(projectId, baseDir) {
    return {
...
export function createInstinctMergeTool(projectId, projectName, baseDir) {
    return {
        name: "instinct_merge",
        ...
        async execute(_toolCallId, params, _signal, _onUpdate, _ctx) {
            ...
            for (const { id, scope } of params.delete_scoped_ids ?? []) {
                ...
                if (id === merged.id && scope === merged.scope)
                    continue;
...
export function registerAllTools(pi, projectId, projectName, baseDir) {
    const guidelines = [
...
//# sourceMappingURL=instinct-tools.js.map