...
import { checkConsolidationGate, countDistinctSessions, loadConsolidationMeta, saveConsolidationMeta, } from "../consolidation.js";
...
import { loadProjectFrequencyTable, saveProjectFrequencyTable, loadGlobalFrequencyTable, saveGlobalFrequencyTable, updateFrequencyTablesFromLines, } from "../prompt-frequency.js";
...
import { generateGlobalSummary, generateProjectSummary, } from "../instinct-summary.js";
...
async function analyzeProject(project, config, baseDir, logger, authStorage) {
    ...
    const { project: updatedProjectFreq, global: updatedGlobalFreq } = updateFrequencyTablesFromLines(newObsLines, projectFreqTable, globalFreqTable);
    ...
    if (isLowSignalBatch(newObsLines, freqContext)) {
        return {
            ran: false,
            skippedReason: "low-signal batch (no errors, corrections, or user redirections)",
        };
    }
    ...
    if (estimatedTotal > PROMPT_TOKEN_BUDGET) {
        ...
        while (promptObsLines.length > 1) {
            const trimmedPrompt = buildSingleShotUserPrompt(project, allInstincts, promptObsLines, {
                agentsMdProject: promptAgentsMdProject,
                agentsMdGlobal: promptAgentsMdGlobal,
                installedSkills,
            });
    ...
    const { apiKey, model, modelId } = await resolveAnalyzerModel(config, authStorage);
    const context = {
    ...
    try {
        ...
        const maxNewInstincts = config.max_new_instincts_per_run ??
        ...
        for (const change of result.changes) {
            if (change.action === "delete") {
                ...
                if (!instinct)
                    continue;
                const dir = instinct.scope === "global"
                    ? globalInstinctsDir
                    : projectInstinctsDir;
...
// Consolidation mode
// ---------------------------------------------------------------------------
async function consolidateProject(project, config, baseDir, logger, force, authStorage) {
    const obsPath = getObservationsPath(project.id, baseDir);
    ...
    if (!force) {
        const gate = checkConsolidationGate({
            ...
            intervalDays: config.consolidation_interval_days ??
                DEFAULT_CONFIG.consolidation_interval_days,
            minSessions: config.consolidation_min_sessions ??
    ...
    saveConsolidationMeta(project.id, {
    ...
    try {
        ...
        if (isConsolidateOnly) {
            ...
            for (const project of projects) {
                try {
                    const result = await consolidateProject(project, config, baseDir, logger, true, authStorage);
            ...
            if (config.dreaming_enabled) {
                for (const project of projects) {
                    try {
                        const result = await consolidateProject(project, config, baseDir, logger, false, authStorage);
                        if (result.ran && result.stats) {
                            processed++;
                            allProjectStats.push(result.stats);
                        }
        ...
        const anyChanges = summary.total_instincts_created > 0 ||
