...
export function invalidateCache(dir) {
    if (dir) {
        dirCache.delete(dir);
    }
    else {
...
function listInstinctsCached(dir) {
    const now = Date.now();
...
function assertNoPathTraversal(id) {
    if (id.includes("..") || id.includes("/") || id.includes("\\")) {
        throw new Error(`Invalid instinct ID "${id}": path traversal characters are not allowed.`);
    }
}
/**
 * Load a single instinct from a .md file path.
 */
export function loadInstinct(filePath) {
 ...
 */
export function saveInstinct(instinct, dir) {
 ...
 */
export function listInstincts(dir) {
    if (!existsSync(dir)) {
        return [];
    }
    const files = readdirSync(dir).filter((f) => f.endsWith(INSTINCT_EXTENSION));
 ...
 */
export function loadProjectInstincts(projectId, baseDir = getBaseDir()) {
 ...
 */
export function loadGlobalInstincts(baseDir = getBaseDir()) {
    const dir = getGlobalInstinctsDir("personal", baseDir);
    return listInstinctsCached(dir);
}
//# sourceMappingURL=instinct-store.js.map