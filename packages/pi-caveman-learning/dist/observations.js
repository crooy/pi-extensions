...
function isFileSizeExceeded(filePath) {
    if (!existsSync(filePath))
        return false;
    return statSync(filePath).size >= MAX_FILE_SIZE_BYTES;
}
function archiveFile(observationsPath, archiveDir) {
 ...
 */
export function appendObservation(observation, projectId, baseDir) {
 ...
 */
export function countObservations(projectId, baseDir) {
...
export function cleanOldArchives(projectId, baseDir) {
    const archiveDir = getArchiveDir(projectId, baseDir);
