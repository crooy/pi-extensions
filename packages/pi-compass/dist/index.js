...
import { handleBeforeAgentStart as buildInjection, } from "./codemap-injector.js";
...
export default function (pi) {
    ...
    pi.on("before_agent_start", (event, _ctx) => {
        try {
            if (!state.project)
                return;
            const result = buildInjection(event, state, DEFAULT_MAX_INJECTION_CHARS);
            if (result) {
    ...
    pi.registerCommand(ONBOARD_CMD, {
        ...
        handler: (args, ctx) => handleOnboardCommand(args, ctx, pi, stateRef),
    });
    pi.registerCommand(TOUR_CMD, {
        description: "Take a guided tour of a specific area of the codebase",
        handler: (args, ctx) => handleTourCommand(args, ctx, pi, stateRef),
    });
}
