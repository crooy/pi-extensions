...
function forgeDir(cwd) {
    return path.join(cwd, FORGE_DIR);
}
function brainstormPath(cwd) {
...
function formatPlanStatus(tasks) {
...
// Injection
// ---------------------------------------------------------------------------
function buildForgeInjection(cwd) {
    const pp = planPath(cwd);
    if (!fs.existsSync(pp))
        return null;
    const content = fs.readFileSync(pp, "utf-8");
...
// Prompts
// ---------------------------------------------------------------------------
const BRAINSTORM_PROMPT = (objective) => `## 🧠 Brainstorm: ${objective}
...
export default function (pi) {
    ...
    pi.registerCommand("forge", {
        description: "Forge: brainstorm → plan → execute. Subcommands: brainstorm, plan, execute, status, next",
            ...
            switch (sub) {
                ...
                case "plan": {
                    if (!fs.existsSync(brainstormPath(ctx.cwd))) {
                        ctx.ui.notify("No brainstorm.md. Run /forge brainstorm <objective> first.", "error");
                        return;
                    }
                ...
                case "execute": {
                    ...
                    pi.sendUserMessage(useTribe ? EXECUTE_TRIBE_PROMPT : EXECUTE_SOLO_PROMPT, { deliverAs: "followUp" });
                    break;
                }
                case "status": {
                    if (!fs.existsSync(planPath(ctx.cwd))) {
                        ctx.ui.notify("No active forge. Use /forge brainstorm <objective> to start.", "info");
                        return;
                    }
                ...
                case "next": {
                    ...
                    pi.sendUserMessage(`## 🔨 Next\n\n${next.text}\n\nMark [x] in plan.md when done.`, { deliverAs: "followUp" });
                    break;
                }
                default: {
                    ...
                    const icon = PHASE_ICONS[phase];
                    ctx.ui.notify(`${icon} Forge: ${phase}\nCommands: brainstorm <obj> | plan | execute [--tribe] | status | next`, "info");
                    break;
                }
            }
        },
