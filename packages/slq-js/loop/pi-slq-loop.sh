#!/usr/bin/env bash
# 🪨 pi-slq-loop — runs slq-js tasks, dispatches to pi worker
set -euo pipefail
...
while true; do
  ...
  # Execute task by dispatching to pi worker
  # If plan exists, include its content in the prompt
  PLAN_CONTENT=""
  if [[ -n "$PLAN_PATH" && -f "$PLAN_PATH" ]]; then
    PLAN_CONTENT=$(cat "$PLAN_PATH")
  fi

  # Dispatch to pi worker via JSON mode
  set +e
