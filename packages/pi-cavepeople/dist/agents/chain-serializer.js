        ...
        if (Array.isArray(parallel)) {
            for (let taskIndex = 0; taskIndex < parallel.length; taskIndex++) {
                ...
                if (!task || typeof task !== "object" || Array.isArray(task))
                    continue;
                const taskErrors = validateAcceptanceInput(task.acceptance, `step ${i + 1} parallel task ${taskIndex + 1} acceptance`);
            ...
            const templateErrors = validateAcceptanceInput(parallel.acceptance, `step ${i + 1} dynamic template acceptance`);
...
//# sourceMappingURL=chain-serializer.js.map