...
import { resolveChainTemplates, createChainDir, removeChainDir, resolveStepBehavior, resolveParallelBehaviors, buildChainInstructions, writeInitialProgressFile, createParallelDirs, suppressProgressForReadOnlyTask, aggregateParallelOutputs, isDynamicParallelStep, isParallelStep, } from "../../shared/settings.ts";
...
import { MAX_CONCURRENCY, resolveChildMaxSubagentDepth, } from "../../shared/types.ts";
...
function ensureParallelProgressFile(chainDir, progressCreated, parallelBehaviors) {
...
function appendParallelWorktreeSummary(output, worktreeSetup, diffsDir, agents) {
    if (!worktreeSetup)
        return output;
...
async function runParallelChainTasks(input) {
    ...
    const parallelResults = await mapConcurrent(input.step.parallel, concurrency, async (task, taskIndex) => {
        ...
        const { prefix, suffix } = buildChainInstructions(behavior, input.chainDir, false, templateHasPrevious ? undefined : input.prev);
        ...
        const effectiveModel = (task.model ? resolveModelCandidate(task.model, input.availableModels, input.ctx.model?.provider) : null)
...
export async function executeChain(params) {
    ...
    let templates = resolveChainTemplates(chainSteps);
    ...
    if (shouldClarify) {
        ...
        const resolvedBehaviors = agentConfigs.map((config, i) => resolveStepBehavior(config, stepOverrides[i], chainSkills));
        const flatTemplates = templates;
        const result = await ctx.ui.custom((tui, theme, _kb, done) => new ChainClarifyComponent(tui, theme, agentConfigs, flatTemplates, originalTask, chainDir, resolvedBehaviors, availableModels, ctx.model?.provider, availableSkills, done), {
            overlay: true,
        ...
        if (isParallelStep(step)) {
            ...
            if (step.worktree) {
                ...
                if (worktreeTaskCwdConflict) {
                    return buildChainExecutionErrorResult(`parallel chain step ${stepIndex + 1}: ${formatWorktreeTaskCwdConflict(worktreeTaskCwdConflict, parallelCwd)}`, makeDetailsInput({ currentStepIndex: stepIndex, currentFlatIndex: globalTaskIndex }));
                }
                try {
            ...
            try {
                ...
                for (let taskIndex = 0; taskIndex < parallelResults.length; taskIndex++) {
                    ...
                    if (outputName)
                        outputs[outputName] = outputEntryFromResult(parallelResults[taskIndex], stepIndex);
                }
                ...
                prev = appendParallelWorktreeSummary(prev, worktreeSetup, path.join(chainDir, "worktree-diffs", `step-${stepIndex}`), agentNames);
            }
            finally {
                if (worktreeSetup)
            ...
            const stepOverride = {
                ...
                skills: tuiOverride?.skills !== undefined
                    ? tuiOverride.skills
            ...
            const { prefix, suffix } = buildChainInstructions(behavior, chainDir, isFirstProgress, templateHasPrevious ? undefined : prev);
...
//# sourceMappingURL=chain-execution.js.map