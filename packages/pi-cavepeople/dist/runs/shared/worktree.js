...
export function resolveExpectedWorktreeAgentCwd(cwd, runId, index) {
    const cwdRelative = resolveRepoCwdRelative(cwd);
...
function createSingleWorktree(toplevel, cwdRelative, runId, index, baseCommit, setupHook, agent) {
...
export function createWorktrees(cwd, runId, count, options) {
    ...
    try {
        for (let index = 0; index < count; index++) {
            worktrees.push(createSingleWorktree(repo.toplevel, repo.cwdRelative, runId, index, repo.baseCommit, setupHook, options?.agents?.[index]));
        }
    }
    catch (error) {
        cleanupWorktrees({
