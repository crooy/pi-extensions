...
import { buildChainInstructions, isDynamicParallelStep, isParallelStep, resolveStepBehavior, suppressProgressForReadOnlyTask, writeInitialProgressFile } from "../../shared/settings.ts";
...
import { ASYNC_DIR, RESULTS_DIR, SUBAGENT_ASYNC_STARTED_EVENT, TEMP_ROOT_DIR, getAsyncConfigPath, resolveChildMaxSubagentDepth, } from "../../shared/types.ts";
...
function resolveJitiCliFromPackageJson(packageJsonPath) {
    ...
    const candidates = [binPath, "lib/jiti-cli.mjs"].filter((candidate) => Boolean(candidate));
...
export function executeAsyncChain(id, params) {
    const { chain, agents, ctx, cwd, maxOutput, artifactsDir, artifactConfig, shareEnabled, sessionRoot, sessionFilesByFlatIndex, maxSubagentDepth, worktreeSetupHook, worktreeSetupHookTimeoutMs, controlConfig, controlIntercomTarget, childIntercomTarget, nestedRoute, } = params;
    ...
    const buildSeqStep = (s, sessionFile, behaviorCwd, progressPrecreated = false, resolvedBehavior) => {
        ...
        return {
            ...
            modelCandidates: buildModelCandidates(behavior.model ?? a.model, a.fallbackModels, availableModels, ctx.currentModelProvider).map((candidate) => applyThinkingSuffix(candidate, a.thinking)),
            tools: a.tools,
    ...
    try {
        steps = chain.map((s, stepIndex) => {
            ...
            if (isDynamicParallelStep(s)) {
                ...
                return {
                    expand: s.expand,
                    parallel: buildSeqStep(s.parallel, undefined, undefined, progressPrecreated, behavior),
                    collect: s.collect,
                    concurrency: s.concurrency,
                    failFast: s.failFast,
                    phase: s.phase,
    ...
    if (spawnResult.pid) {
        ...
        ctx.pi.events.emit(SUBAGENT_ASYNC_STARTED_EVENT, {
            ...
            chain: chain.map((s) => isParallelStep(s) ? `[${s.parallel.map((t) => t.agent).join("+")}]` : isDynamicParallelStep(s) ? `expand:${s.parallel.agent}` : s.agent),
            chainStepCount: chain.length,
            parallelGroups,
            workflowGraph,
            cwd: runnerCwd,
...
export function executeAsyncSingle(id, params) {
    const { agent, agentConfig, ctx, cwd, maxOutput, artifactsDir, artifactConfig, shareEnabled, sessionRoot, sessionFile, maxSubagentDepth, worktreeSetupHook, worktreeSetupHookTimeoutMs, controlConfig, controlIntercomTarget, childIntercomTarget, nestedRoute, } = params;
    ...
    const model = applyThinkingSuffix(resolveModelCandidate(params.modelOverride ?? agentConfig.model, availableModels, ctx.currentModelProvider), agentConfig.thinking);
    let spawnResult = {};
    try {
        spawnResult = spawnRunner({
            id,
            steps: [
                {
                    ...
                    modelCandidates: buildModelCandidates(params.modelOverride ?? agentConfig.model, agentConfig.fallbackModels, availableModels, ctx.currentModelProvider).map((candidate) => applyThinkingSuffix(candidate, agentConfig.thinking)),
...
//# sourceMappingURL=async-execution.js.map