...
import { DEFAULT_MAX_OUTPUT, truncateOutput, getSubagentDepthEnv, } from "../../shared/types.ts";
import { DEFAULT_CONTROL_CONFIG, buildControlEvent, deriveActivityState, claimControlNotification, formatControlIntercomMessage, formatControlNoticeMessage, } from "../shared/subagent-control.ts";
import { isDynamicRunnerGroup, isParallelGroup, flattenSteps, mapConcurrent, aggregateParallelOutputs, MAX_PARALLEL_CONCURRENCY, } from "../shared/parallel-utils.ts";
...
import { createMutatingFailureState, didMutatingToolFail, isMutatingTool, nextLongRunningTrigger, recordMutatingFailure, resetMutatingFailureState, resolveCurrentPath, shouldEscalateMutatingFailures, summarizeRecentMutatingFailures, } from "../shared/long-running-guard.ts";
...
function runPiStreaming(args, cwd, outputFile, env, piPackageRoot, piArgv1, maxSubagentDepth, childEventContext, registerInterrupt, onChildEvent) {
...
async function runSingleStep(step, ctx) {
    ...
    for (let index = 0; index < candidates.length; index++) {
        ...
        const run = await runPiStreaming(args, step.cwd ?? ctx.cwd, ctx.outputFile, env, ctx.piPackageRoot, ctx.piArgv1, step.maxSubagentDepth, { eventsPath, runId: ctx.id, stepIndex: ctx.flatIndex, agent: step.agent }, ctx.registerInterrupt, ctx.onChildEvent);
        ...
        if (isDynamicRunnerGroup(step)) {
            ...
            if (materialized.parallel.length === 0) {
                ...
                const groupAcceptance = step.effectiveAcceptance?.explicit
                    ...
                    : undefined;
                if (placeholder && groupAcceptance)
                    placeholder.acceptance = groupAcceptance;
            ...
            const dynamicStatusSteps = dynamicSteps.map((task) => ({
                agent: task.agent,
