...
import { clearSlashSnapshots, getSlashRenderableSnapshot, resolveSlashMessageDetails, restoreSlashFinalSnapshots } from "../slash/slash-live-state.ts";
...
import { ASYNC_DIR, DEFAULT_ARTIFACT_CONFIG, RESULTS_DIR, SLASH_RESULT_TYPE, SUBAGENT_ASYNC_COMPLETE_EVENT, SUBAGENT_ASYNC_STARTED_EVENT, SUBAGENT_CONTROL_EVENT, WIDGET_KEY, } from "../shared/types.ts";
import { clearPendingForegroundControlNotices, formatSubagentControlNotice, handleSubagentControlNotice, SUBAGENT_CONTROL_MESSAGE_TYPE, } from "./control-notices.ts";
...
function createSlashResultComponent(details, options, theme) {
    ...
    const promptTemplateBridge = registerPromptTemplateDelegationBridge({
        ...
        execute: async (requestId, request, signal, ctx, onUpdate) => {
            if (request.tasks && request.tasks.length > 0) {
                return executeSubagentCollapsed(requestId, {
                    tasks: request.tasks,
...
• { action: "doctor" } - read-only report for runtime paths, discovery, sessions, and intercom`,
        ...
        renderCall(args, theme) {
            ...
            if (args.chain?.length)
                return new Text(`${theme.fg("toolTitle", theme.bold("subagent "))}chain (${args.chain.length})${asyncLabel}`, 0, 0);
            if (isParallel)
