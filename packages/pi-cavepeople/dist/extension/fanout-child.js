...
function startNestedControlInboxListener(pi, state) {
    let route;
    try {
                        ...
                        if (!result) {
                            ...
                            try {
                                ...
                                if (!control) {
                                    ...
                                    ok = await deliverSubagentIntercomMessageEvent(pi.events, target, `Follow-up for nested run ${request.targetRunId} (${control.currentAgent}):\n\n${request.message.trim()}`, 500, { source: "nested-resume", runId: request.targetRunId, agent: control.currentAgent, index });
                                    message = ok
...
//# sourceMappingURL=fanout-child.js.map