...
import { handleBeforeAgentStart as buildInjection, } from "./tdd-injector.js";
import { updateStatusBar } from "./status-bar.js";
import { handleTddCommand, COMMAND_NAME as TDD_CMD } from "./tdd-command.js";
import { handleTddStatusCommand, COMMAND_NAME as TDD_STATUS_CMD, } from "./tdd-status-command.js";
...
export default function (pi) {
    ...
    pi.on("before_agent_start", (event, _ctx) => {
        try {
            if (!state || !config)
                return;
            return buildInjection(event, state, config) ?? undefined;
        }
        catch (err) {
        ...
        try {
            ...
            if (toolName === "Write" || toolName === "Edit") {
                ...
                if (filePath) {
                    ...
                    else if (classification === "implementation" &&
                ...
                if (command && isTestRunCommand(command, config)) {
                    ...
                    if (testResult) {
                        ...
                        if (config.auto_advance) {
                            const trigger = updatedResult.errors > 0
                                ? "tests_error"
                            ...
                            if (state.phase !== previousPhase) {
                                ctx.ui.notify(`TDD phase: ${previousPhase.toUpperCase()} -> ${state.phase.toUpperCase()}`, "info");
                                updateStatusBar(ctx, state);
                            }
                        }
                        saveState(state);
    ...
    pi.registerCommand(TDD_CMD, {
        ...
        handler: (args, ctx) => handleTddCommand(args, ctx, stateRef, pi),
    });
    pi.registerCommand(TDD_STATUS_CMD, {
        description: "Show detailed TDD status",
        handler: (args, ctx) => handleTddStatusCommand(args, ctx, stateRef),
    });
}
...
function extractBashResult(result) {
