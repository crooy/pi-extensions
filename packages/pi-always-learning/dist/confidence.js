 ...
 */
export function confirmationDelta(confirmedCount) {
    if (confirmedCount <= CONFIRMED_TIER1_MAX)
        return DELTA_CONFIRMED_TIER1;
    if (confirmedCount <= CONFIRMED_TIER2_MAX)
        return DELTA_CONFIRMED_TIER2;
 ...
 */
export function adjustConfidence(current, outcome, confirmedCount = 0) {
    let delta;
