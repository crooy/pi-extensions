...
export function createFactWriteTool(projectId, projectName, baseDir) {
    return {
        name: "fact_write",
        label: "Write Fact",
        description: "Create or update a knowledge fact (declarative statement)",
        promptSnippet: "Create or update a fact — a declarative knowledge note with no trigger/action structure",
        parameters: WriteParams,
        async execute(_toolCallId, params, _signal, _onUpdate, _ctx) {
            const dir = getFactsDir(params.scope, projectId, baseDir);
            if (!dir) {
                throw new Error("Cannot write project-scoped fact: no project detected");
            }
...
export function createFactReadTool(projectId, baseDir) {
    return {
        name: "fact_read",
        label: "Read Fact",
        ...
        async execute(_toolCallId, params, _signal, _onUpdate, _ctx) {
            ...
            return {
                content: [{ type: "text", text: formatFact(fact) }],
                details: fact,
            };
        },
    };
}
export function createFactListTool(projectId, baseDir) {
    return {
        name: "fact_list",
        label: "List Facts",
        description: "List knowledge facts with optional filters",
        promptSnippet: "List all knowledge facts, optionally filtered by scope or domain",
...
export function createFactDeleteTool(projectId, baseDir) {
    return {
        name: "fact_delete",
        label: "Delete Fact",
        ...
        async execute(_toolCallId, params, _signal, _onUpdate, _ctx) {
            if (params.scope) {
                ...
                if (!existsSync(path)) {
                    throw new Error(`Fact not found: ${params.id} in ${params.scope} scope`);
                }
                unlinkSync(path);
                return {
                    content: [
...
export function registerAllFactTools(pi, projectId, projectName, baseDir) {
    const guidelines = [
