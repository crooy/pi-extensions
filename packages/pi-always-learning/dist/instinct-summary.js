...
import { getBaseDir, getProjectsRegistryPath, getGlobalSummaryPath, getProjectSummaryPath, } from "./storage.js";
...
function formatTable(instincts) {
    const sorted = [...instincts].sort((a, b) => b.confidence - a.confidence);
    const rows = sorted.map((i) => `| ${i.id} | ${i.title} | ${i.confidence.toFixed(2)} | ${i.domain} |`);
    return [
 ...
 */
export function generateGlobalSummary(baseDir = getBaseDir()) {
    ...
    for (const { project, instincts } of projectEntries) {
        lines.push("", `## Project: ${project.name} (${plural(instincts.length, "instinct")})`, "");
 ...
 */
export function generateProjectSummary(projectId, baseDir = getBaseDir()) {
    ...
    try {
        writeFileSync(getProjectSummaryPath(projectId, baseDir), lines.join("\n") + "\n", "utf-8");
    }
    catch {
        // Best effort - don't crash callers on I/O failure
    }
}
//# sourceMappingURL=instinct-summary.js.map