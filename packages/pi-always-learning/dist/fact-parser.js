...
function assertKebabCase(id) {
    if (!KEBAB_RE.test(id)) {
        throw new Error(`Invalid fact ID "${id}": must be kebab-case (lowercase letters, numbers, hyphens only).`);
    }
}
function splitFrontmatter(content) {
    const match = content.match(/^---\r?\n([\s\S]*?)\r?\n---\r?\n?([\s\S]*)$/);
    if (!match) {
        throw new Error("Invalid fact file: content must begin with YAML frontmatter delimiters (---).");
    }
    return { frontmatterStr: match[1] ?? "", body: (match[2] ?? "").trim() };
}
export function parseFact(content) {
    const { frontmatterStr, body } = splitFrontmatter(content);
    const fm = parseYaml(frontmatterStr);
    if (!fm || typeof fm !== "object") {
        throw new Error("Invalid fact file: frontmatter is not a valid YAML object.");
    }
    for (const field of REQUIRED_FIELDS) {
        if (fm[field] === null || fm[field] === undefined) {
            throw new Error(`Invalid fact file: missing required field "${field}".`);
        }
    }
    ...
    if (Array.isArray(fm["evidence"])) {
        fact.evidence = fm["evidence"].map(String);
    }
    if (fm["flagged_for_removal"] !== undefined &&
...
//# sourceMappingURL=fact-parser.js.map