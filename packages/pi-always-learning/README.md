![pi-always-learning banner](docs/images/banner.png)
...
Pi is smart — but it starts every session with no memory of what worked yesterday. `pi-always-learning` fixes that.
...
```bash
pi install npm:pi-always-learning
```
...
| Node.js | >= 18 |
| LLM provider | configured in Pi (analyzer uses default model; provider/model are configurable) |
...
The background analyzer (`pi-cl-analyze`) processes new observations using the configured provider/model (uses Pi default model). It applies three tiers of quality filtering:
...
| `model` | *(default model)* | Model for the background analyzer (uses Pi default if unset) |
| `provider` | *(default provider)* | Pi provider for the background analyzer (uses Pi default if unset) |
